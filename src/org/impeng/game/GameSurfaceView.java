...
public class GameSurfaceView extends SurfaceView implements Runnable,SurfaceHolder.Callback {
	...
	public void checkCollision() {
		...
		Enemy n = null;
		Gift g = null;
		for(int i = 0; i < pool.bulletList.size(); i++)	{ //player子弹与enemy的碰撞检测
		...
		for(int i = 0; i < enemyArray.size(); i++){  //player与enemy的碰撞检测
			n = enemyArray.get(i);
		...
		for(int i = 0; i < enemy_pool.bulletList.size(); i++)	{ //player与enemy的子弹碰撞检测
		...
		for(int i = 0; i < giftArray.size(); i++)	{ //player与enemy的子弹碰撞检测
			g = giftArray.get(i);
